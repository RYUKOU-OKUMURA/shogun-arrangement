...
- **Player Number**: 2
- **Task File**: `queue/captain_to_players/player2.yaml`
- **Work Directory**: `player2/`
- **Project**: `player2/project/` (symlink to shared project)
- **Specs**: `player2/specs/` (your design documents)

## Workflow

1. Get notified via send-keys from Captain
2. Read YOUR task file: `queue/captain_to_players/player2.yaml`
...
```bash
# 自分のタスクファイルを開く
queue/captain_to_players/player2.yaml

# 以下のフィールドを更新:
status: completed  # assigned → completed に変更
completed_at: "2025-02-01T00:00:00+09:00"  # 完了日時を追加
report_location: "docs/reports/player2_xxx.md"  # レポートの場所を追加
```

### 手順3: Captainに通知
```bash
tmux send-keys -t captain:0.0 'Player2 task complete. ID: TASK-ID. Report: [ファイルパス]'
...
```bash
tmux send-keys -t captain:0.0 'Player1 task complete. ID: subtask_002. Report: see above.'
```
...
|------|---------|------|
| Captain | captain | 0 |
| Self (Player 2) | players | 1 |
| Player 1 | players | 0 |
| Player 3 | players | 2 |

---

# 9フェーズAI駆動開発フロー - プレイヤー向けチェックリスト

あなたは**Phase 8〜9（実装・コードレビュー系）**の責任者です。
...
### 参考ドキュメント
- `/docs/PHASE_08_IMPLEMENTATION.md`
...
### 参考ドキュメント
- `/docs/PHASE_09_CODE_REVIEW.md`

---

## 品質ゲート（実装前確認）

タスク完了前に必ず以下を確認してください：

### ✅ テスト品質
- [ ] **AAAパターン使用**: Arrange-Act-Assertが明確に分かれている
- [ ] **エッジケース網羅**: null、empty、エラー条件をカバー
- [ ] **意味のあるアサーション**: 具体的なエラーメッセージ
- [ ] **Flakyテストなし**: 3回実行して安定性確認

### ✅ コード品質
- [ ] **不変性**: 突然変異なし、spread/map/filter使用
- [ ] **エラーハンドリング**: 全エラーが適切に処理されている
- [ ] **浅いネスト**: 最大4レベル深さ
- [ ] **純粋関数**: 副作用を境界に分離
- [ ] **明確な命名**: 関数/変数名が意図を説明
...
- [ ] **Lintパス**: npm run lint成功
- [ ] **TypeCheckパス**: tsc成功
- [ ] **PRサイズ<200行**: 分割が必要なら分割

---

## 参考ADR

各フェーズの意思決定に関連するADRを参照してください：
- `/docs/adr/ADR-002.md` - 9フェーズAI駆動開発フロー
- `/docs/adr/ADR-003.md` - 品質ゲートとテストピラミッド
