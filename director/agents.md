...
# 9フェーズAI駆動開発フロー - ディレクター向けチェックリスト

あなたは**Phase 1〜5（計画・設計系）**の責任者です。
...
### 参考ドキュメント
- `/docs/PHASE_02_INVESTIGATION.md`
- `/docs/adr/ADR-XXX.md`（関連ADR）
...
| 項目 | 0→1（新規開発） | 1→2（機能追加・改修） |
|------|----------------|---------------------|
| 実行フェーズ | 全9フェーズ | 副作用分析重視 |
| タスク分解 | 少しだけ大きめ | 最小単位で |
| スキップ可能 | なし | 軽微な変更ならタスク分解をスキップ可能 |
...
- `/docs/adr/ADR-000.md` - ADR導入の決定
- `/docs/adr/ADR-001.md` - マルチエージェントアーキテクチャ
